...
bool network_storage::erase(uintptr_t address) {
    callstack_container callstack;
    net_request request(net_req_erase, address, 0, boost::chrono::system_clock::now(),
        callstack);
    send(request);
    return true;
}
...
bool network_storage::update_size(uintptr_t address, std::size_t memsize) {
    callstack_container callstack;
    net_request request(net_req_upd_size, address, memsize, boost::chrono::system_clock::now(),
        callstack);
    send(request);
    return true;
}
