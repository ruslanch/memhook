...
namespace memhook {
  std::string FileBasedMappedStorageCreatorMixin::GenerateUniquePath(
    ...
    const std::string pid_extension =
            "." + boost::lexical_cast<std::string>(guide ? guide : ::getpid());
    const std::string unique_path_base =
            (orig_path.parent_path() / orig_path.stem()).string() + pid_extension;
