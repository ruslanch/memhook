...
int main(int argc, char const *argv[])
{
    ...
    try
    {
        ...
        if (options_map.count("mapped-file"))
        {
            ...
            unique_ptr<MappedStorageCreator> tmp(NewMMFMappedStorageCreator(file_path.c_str(),
        ...
        else
        {
            ...
            unique_ptr<MappedStorageCreator> tmp(NewSHMMappedStorageCreator(name.c_str(),
                options_map["size"].as<std::size_t>()));
            storage_creator.reset(tmp.release());
        }
        ...
        MemDBServer server(storage_creator, io_service,
