...
namespace memhook {
  ...
  void MemDBSession::Start() {
    ...
    boost::asio::async_read(
            socket_,
            sbuf_,
            boost::asio::transfer_at_least(outbound_size),
            boost::bind(&MemDBSession::OnReadComplete,
                    shared_from_this(),
                    ParsingCallback(boost::bind(&MemDBSession::ParseOutbound, this)),
                    boost::asio::placeholders::error));
  }

  void MemDBSession::OnReadComplete(
          const ParsingCallback &callback, const boost::system::error_code &e) {
    if (e) {
    ...
    boost::asio::async_read(socket_,
            sbuf_,
            boost::asio::transfer_exactly(parsing_info.size),
            boost::bind(&MemDBSession::OnReadComplete,
                                    shared_from_this(),
  ...
  bool MemDBSession::HandleNetRequest(NetRequest &request) {
    switch (request.type) {
      case kNetReqAdd:
        OnAdd(request.traceinfo, request.callstack);
        return true;
      case kNetReqRemove:
        OnRemove(request.traceinfo);
        return true;
      case kNetReqUpdateSize:
        OnUpdateSize(request.traceinfo);
        return true;
      case kNetReqNewStorage:
  ...
  void MemDBSession::OnAdd(const TraceInfoBase &traceinfo, const CallStackInfo &callstack) {
    storage_->Add(traceinfo.address, traceinfo.memsize, callstack, traceinfo.timestamp);
  }

  void MemDBSession::OnRemove(const TraceInfoBase &traceinfo) {
    storage_->Remove(traceinfo.address);
  }
