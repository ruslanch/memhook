...
struct ReserveSeqCapacity<boost::container::basic_string<T, U, A> >
{
    ...
    static void Call(Seq &seq, std::size_t size) { seq.reserve(size); }
};
...
struct ReserveSeqCapacity<boost::unordered_map<K, V, H, E, A> >
{
    ...
    static void Call(Seq &seq, std::size_t size) { seq.rehash(ceil(size / seq.max_load_factor())); }
};
...
struct ByteOrderingConv<T, typename boost::enable_if_c<sizeof(T) == sizeof(uint8_t)>::type> {
    ...
    static BOOST_FORCEINLINE BOOST_CONSTEXPR Type hton(Type val) { return val; }
    static BOOST_FORCEINLINE BOOST_CONSTEXPR Type ntoh(Type val) { return val; }
};
...
struct ByteOrderingConv<T, typename boost::enable_if_c<sizeof(T) == sizeof(uint16_t)>::type>
{
    ...
    static BOOST_FORCEINLINE BOOST_CONSTEXPR Type hton(Type val) { return hton_16(val); }
    static BOOST_FORCEINLINE BOOST_CONSTEXPR Type ntoh(Type val) { return ntoh_16(val); }
};
...
typename ByteOrderingConv<T>::Type hton(T val)
{
    return ByteOrderingConv<T>::hton(static_cast<typename ByteOrderingConv<T>::Type>(val));
}

template <typename T> BOOST_FORCEINLINE BOOST_CONSTEXPR
typename ByteOrderingConv<T>::Type ntoh(T val) {
...
template <typename T> inline
void Write(boost::asio::mutable_buffer &buf, const T &val)
{
    BufferWriter h(buf);
    boost::fusion::for_each(val, h);
}
...
template <typename T> inline
void Read(boost::asio::const_buffer &buf, T &val)
{
    BufferReader h(buf);
    boost::fusion::for_each(val, h);
}
